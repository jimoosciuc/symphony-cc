...
| `RateLimitEvent`                                                    | `heartbeat`                                | `{ "kind": "rate_limit", "data": {...} }`. Distinct kind so operators and tests can filter rate-limit signals without parsing nested SDK payloads. |
| `StreamEvent`, `TaskStartedMessage`, `TaskProgressMessage`, `TaskNotificationMessage`, `HookEventMessage` | `heartbeat`                | `{ "kind": "<message_class>", "data": {...} }`. Generic SDK liveness/progress signals; collapsed into one `heartbeat` event to satisfy stall detection without leaking SDK shapes. |

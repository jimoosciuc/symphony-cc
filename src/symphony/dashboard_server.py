...
_REDACT_KEYS = ("token", "authorization", "api_key", "password", "secret", "git_token")
...
class DashboardServer:
    ...
    def start(self) -> None:
        ...
        self._server = _DashboardHTTPServer((self._host, self._port), handler)
        self._running = True
        self._thread = Thread(target=self._serve, daemon=True, name="symphony-dashboard")
        self._thread.start()
        logger.info("Dashboard server started at http://%s:%s", self._host, self.port)
    ...
    def _make_handler(self) -> type[BaseHTTPRequestHandler]:
        ...
        def snapshot() -> dict[str, Any]:
            return redact(status_provider(), redact_keys=_REDACT_KEYS)
